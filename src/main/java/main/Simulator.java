...
import service.TodoItemServiceTemplate;
import service.UserServiceTemplate;
import ui.Font;
import ui.Text;
...
public class Simulator {
    ...
    TodoItemServiceTemplate itemsService;
    ...
    private UserServiceTemplate userService;
    @Autowired
    public Simulator(UserServiceTemplate userService,TodoItemServiceTemplate itemsService){
    ...
    private void getUserUpdatedData(){
        currentUser.setItems(userService.getUserTodos(currentUser.getName()));
    }
    public ArrayList<User> loadData() {
        ArrayList<User> users = new ArrayList<>();
        ArrayList<String> usernames = userService.getUserNames();
        for (String username : usernames) {
            ArrayList<TodoItem> userItems = userService.getUserTodos(username);
            User user = userService.setUserData(username, userItems);
            users.add(user);
        }
        return users;
    }
    ...
    private void showMenu() {
        ...
        while (true) {
            ...
            switch (option) {
                ...
                case 5:
                    itemsService.showTop5ItemsByDate(currentUser.getName());
                    break;
    ...
    private String getExistingTitle(String messageSpecifier) {
        ...
        while (true) {
            ...
            if (title.equalsIgnoreCase("/back")) return title;
            if (itemsService.itemExists(title,currentUser.getItems())) break;
   ...
   private String getOldTitleFromUser() {
        while (true) {
            ...
            if (itemsService.itemExists(oldTitle,currentUser.getItems())) {
                return oldTitle;
            }
    ...
    private void takeUpdateItemFromUser() {
        ...
        if (updated) {
            itemsService.updateTodoItem(currentUser.getName(),item,oldTitle);
